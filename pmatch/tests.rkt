...
(check-equal? (pmatch '(?x ?op ?y (?if (?op ?x ?y))) '(4 > 3))
              '((?y . 3) (?op . >) (?x . 4)))
(check-equal? (pmatch '(i love lisp) '(i hate lisp) fail) '())
(check-equal? (pmatch '(i love lisp) '(i hate lisp)) '())
(check-equal? (pmatch '() '(i hate lisp)) '((t . t)))
(check-equal? (pmatch '(i love lisp) '(i hate lisp)) '())
(check-equal? (pmatch '(?X loves ?Y) '(Bharath loves lisp))
              '((?Y . lisp) (?X . Bharath)))
(check-equal? (pmatch '((?* ?X) loves ?Y) '(Bharath really loves lisp))
